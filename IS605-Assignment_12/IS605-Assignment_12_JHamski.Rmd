...
cv.err <- rbind(cv.err.1, cv.err.2,cv.err.3,cv.err.4,cv.err.5,cv.err.6,cv.err.7,cv.err.8)
```
...
sapply(glm.fit.all, FUN=cross.validate)


cross.validate(glm.fit.all[[1]])

cv.err<- cv.glm(cars, glm.fit)$delta

cv.err5[1] <- cv.glm(cars,glm.fit.1,K=5)$delta[1]
cv.err5[2] <- cv.glm(cars,glm.fit.2,K=5)$delta[1]
